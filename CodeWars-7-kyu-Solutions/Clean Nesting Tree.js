...
function isCleanlyNested(xs) {
  ...
  const childTypes = xs.map((child) =>
    Array.isArray(child) && child.length === 0 ? 'deadend' : 'nested'
  );

  const allSameType = childTypes.every((type) => type === childTypes[0]);
  if (!allSameType) return false;

  // Recursively check nested children
  return xs.every((child) => isCleanlyNested(child));
}
